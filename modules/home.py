...
def get_upcoming_contests():
    ...
    for i in range(6):
        ...
        y = cur.year + (m - 1) // 12
        m = ((m - 1) % 12) + 1
        contests.append({'name': 'Codeforces Round ≈6-8 场', 'date': date(y, m, 15), 'type': 'CF'})
        contests.append({'name': 'AtCoder 常规赛 ≈4 场', 'date': date(y, m, 20), 'type': 'AT'})
        contests.append({'name': '洛谷周赛 ≈4 场', 'date': date(y, m, 25), 'type': '洛谷'})
